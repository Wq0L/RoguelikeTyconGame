...
public class ComicHoverMotion : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler, ISelectHandler, IDeselectHandler
{
    ...
    void OnDisable(){if(animation!=null)StopCoroutine(animation);animation=null;over=selected=down=false;transform.localScale=restScale;transform.localRotation=restRotation;}
    void Animate(){
        ...
        if(animation!=null)StopCoroutine(animation);animation=StartCoroutine(Tween(scale,angle));
    }
    IEnumerator Tween(float scale,float angle){
        ...
        transform.localScale=restScale*scale;transform.localRotation=end;animation=null;
    }
